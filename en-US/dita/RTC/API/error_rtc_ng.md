...
This article provides descriptions and troubleshooting tips for common Agora SDK error and warning codes. If you receive a response that is not included here, Agora recommends you [submit ](https://agora-ticket.agora.io/)a ticket so that our technical support team can help resolve the issue.
...
| `5` | The request is rejected. 可能的原因有：[IRtcEngine] 初始化失败。 Please re-initialize [IRtcEngine]. |
...
| `102` | The channel name is not valid. 可能的原因是设置的参数数据类型不正确。 Please rejoin the channel with a valid channel name. |

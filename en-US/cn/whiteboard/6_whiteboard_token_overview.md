...
The whiteboard service has three types of Tokens: SDK Token, Room Token, and Task Token in descending order of granted permissions. Each Token can be assigned an `admin`, `writer`, or `reader` role. Token 的级别越高，包含的权限越多。 Such Tokens need to be issued from your app server.

### SDK Token

A SDK Token is linked with a whiteboard project at Agora Console and grants the most permissions. With a SDK Token, a user can get access to every room and file conversion task under the linked project. The permissions granted by a SDK Token in a specific role are as follows:

| 权限 | admin Task Token | writer Task Token | reader Task Token |
| :----------------------------------------------------------- | :--------------------------- | :--------------------------- | :--------------------------- |
...
| Generate a Room Token in a same or inferior role (for example, an admin SDK Token can generate an  `admin`, `writer`, or `reader` Room Token) | <font color="green">✔</font> | <font color="green">✔</font> | <font color="green">✔</font> |
| Generate a Task Token in a same or inferior role (for example, an admin SDK Token can generate an  `admin`, `writer`, or `reader` Task Token) | <font color="green">✔</font> | <font color="green">✔</font> | <font color="green">✔</font> |
...
<ul>
	<li>Do not send SDK Tokens to your app clients, save them in a database, or write them in a configuration file. You should issue them from your app server only when it is necessary.</li>
...
A Room Token is linked with a room under a whiteboard project at Agora Console. With a Room Token, a user can get access to the linked room. The permissions granted by a Room Token in a specific role are as follows:
...
A Task Token is linked with a file conversion task under a whiteboard project at Agora Console. With a Task Token, a user can get access to the linked  task. The permissions granted by a Task Token in a specific role are as follows:
...
You can generate a Token for the whiteboard service through one of the following ways:

- Go[ to Agora](https://console.netless.link/zh-CN/) Console. See[ Get a SDK Token](/cn/whiteboard/enable_whiteboard).

<div class="alert note">In this way, you can only get a permanent<code> admin</code> SDK Token. Do not send it to your app clients; otherwise there may be a risk of exposure.</div>
...
- The Room UUID (for Room Token only)
- The Task UUID (for Task Token only)

### Get access keys

The access keys consist of an AK (Access Key) and a SK (Secret Key). Follow the steps to get the access keys:
...
<li>Contact support@agora.io to get new access keys in case of a risk of exposure.</li></ul></div>

### Set Token role

A Token can be assigned an `admin`, `writer`, or `reader` role. The permissions granted by each Token role are described in<a href="#tokenoverview"></a>Introduction.


<div class="alert note">To enhance security, Agora recommends that you:
<ul>
	<li>Avoid sending Tokens that grant a lot of permissions to your app clients.</li>
	<li> Do not use Tokens that grant more permissions than is needed in your app scenario when it is necessary to send one to an app client.</li>
	</ul>
</div>

### Set validity period

The Token validity periods are set as positive integers in milliseconds. You can get the expiration time by adding up the validity period and the UTC time when the Token is generated. Once a Token expires, users cannot use it to join a room or access the whiteboard service. To ensure availability, you should generate new Tokens on your app server in a timely manner.

<div class="alert info">If a user uses a Token to join a room and the Token expires when the user is still in the room, the user will not be kicked out of the room.</div>

Not setting the validity period or setting it to 0 generates a permanent Token,

<div class="alert note">which may cause security risks to your app service. Suppose an illegal user gets a permanent Token that grants a lot of permissions, and uses it to damage your app system. The only way to invalidate the Token is to disable the access keys used to generate it, but this action has significant side effects. Therefore, Agora recommends that you make a maximum estimate of how long the Token will be used on app clients based on your app scenario, and set it as the validity period.</div>

### Get Room UUID

To generate a Room Token, you need to pass in the Room UUID, the unique identifier of a whiteboard room, so that the Room Token is linked with the room. Follow the steps to get a Room UUID:
...
If you generate the Room Token by adding code on the app server, the access keys passed in must be the same as that used to generate the SDK Token for creating the room.
</div>
...
The following examples explain how Tokens are used step by step in the process:

**Example 1: An app client requests to join a room**

1. The app server calls the interactive whiteboard RESTful API or uses code to generate a SDK Token.
...
| `expired token` | Occurs when the token expires. Please call the interactive whiteboard RESTful API or use code to generate a new Token from the app server. |

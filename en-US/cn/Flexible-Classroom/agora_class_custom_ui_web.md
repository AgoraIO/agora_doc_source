...
Agora provides a complete[ Agora Classroom SDK on npm](https://www.npmjs.com/package/agora-classroom-sdk) for you to integrate. However, if the default UI in the Agora Classroom SDK cannot meet your needs, you can also obtain the source code of the Agora Classroom SDK to develop, debug, and compile by yourself. The source code of the Agora Classroom SDK is located in the [CloudClass-Desktop ](https://github.com/AgoraIO-Community/CloudClass-Desktop/tree/dev/apaas%2F1.1.0)repository on GitHub (release/apaas/1.1.0 branch).

In the Agora Classroom SDK, the UI code of the Flexible Classroom Classroom is separated from the core business logic, and becomes the UI Kit and the Edu Core independently, and the two are related through the Agora Edu Context. For example, for the text chat function in the  Flexible Classroom, you need to send a message through a button, and at the same time you need to receive messages sent by other users. In this case, we can call the sending message method in the Chat Context in the UI Kit, and monitor the message reception related events in the Chat Context. If you need to modify the style of the button,

![](https://web-cdn.agora.io/docs-files/1619696813295)

UI Kit provides the UI component code of the  Flexible Classroom, and introduces the open source tool [Storybook](https://storybook.js.org/docs/react/get-started/introduction) to develop and manage UI components. If developers only want to modify the classroom UI, they can modify only the UI components without in-depth study of the core business logic details of the smart Flexible Classroom.

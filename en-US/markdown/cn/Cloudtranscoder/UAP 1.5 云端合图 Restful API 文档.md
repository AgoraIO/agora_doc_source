...
- `The 202<code> status code only indicates that the server has received the task request, but it does not mean that the execution is complete. You need to continue to query the execution status through the `Query</code> method to determine whether the task has been executed.
...
- After receivingthe `5xx` response status code, it is generally because the server has a problem in the response process. It is recommended to use the backoff algorithm (for example, 5 seconds, 10 seconds, 15 seconds) to call the `Query` request for confirmation.
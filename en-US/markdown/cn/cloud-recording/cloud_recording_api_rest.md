...
If this method call succeeds, you get `a resource ID (resourceId`) from the HTTP response body. The resource ID is valid for five minutes, so you need to start recording with this resource ID within five minutes[](#start).
...
| :------ | :----- | :----------------------------------------------------------- |
| `appId` | String | App ID](https://docs.agora.io/cn/Agora%20Platform/terms?platform=All%20Platforms#appid) of your project[.<ul><li>For page recording, simply enter the App ID that has enabled the  Cloud Recording service.</li><li>In other cases, you must use the same App ID as the channel to be recorded, and the App ID needs to be enabled for  Cloud Recording service.</li></ul> |

The following parameters are required in the URL.

| Parameter | Category | Description |
| :-------------- | :----- | :----------------------------------------------------------- |
| `cname` | String | <ul><li> In non-page recording mode, this parameter is used to set the name of the channel to be recorded.</li><li>For page recording, this parameter is used to differentiate the recording process. The maximum length of this parameter is 1024 bytes. A unique` recording instance` can be located by appid`, cname` and `uid`. If you want to record the same page multiple times, you can use the same `cname` and a different `uid` to organize and differentiate.</li></ul> |
| `uid` | String | A string that contains the UID of the recording client, for example `"527841"`. The UID needs to meet the following requirements:<ul><li>It is a <sup>32</sup>-bit unsigned integer within the range between 1 and (232-1).</li><li><b><em>It is unique and does not clash with any existing UID in the channel.</em></b></li><li>It should not be a string. Ensure that all UIDs in the channel are integers.</li></ul> |
| `clientRequest requires the following parameters:` | JSON | A specific client request that requires the following parameters:<ul><li> `A client request including the` resourceExpiredHour` field. resourceExpiredHour is an integer, which sets the time limit (in hours) for the Cloud Recording RESTful API calls. It must be between 1 and 720, the default value being 72`. The time limit starts from when you successfully start a `cloud recording `and `get sid`(the `recording ID` ). `When you exceed the time limit,` you can`` no longer `call query, updateLayout`, and `stop`.</li><li>`scene`: (optional) Number type, used to set the scene of the  Cloud Recording resource use.<ul><li>`0`: (Default) Enables both audio and video recording. After setting up this scene, the recording service will upload the recording files to your specified third-party cloud storage in real time.</li><li>`1`: Page recording.</li><li>`2`: Time-delay transcoding. After setting this scene, the recording service will transcode the recorded files to generate MP4 files within 24 hours after recording and upload the MP4 files to the third-party cloud storage you specify (7Niu cloud is not supported). This scenario is only available for  Individual Recording mode. You need to` set the `appsCollection `parameter in the start `method as well.</li></ul></li></ul> |
...
- `resourceId`: String. The resource ID for cloud recording. You need to start recording within five minutes after getting a resource ID.

## <a name="start"></a>Starts cloud recording

After getting a resource ID, call this method to start cloud recording. The HTTP method and endpoint of start:
...
`recordingConfig` 是一个用于设置媒体流订阅的 JSON Object。 云端录制会根据此设置订阅频道内的媒体流，并生成录制文件或截图。 `recordingConfig` has the following fields:

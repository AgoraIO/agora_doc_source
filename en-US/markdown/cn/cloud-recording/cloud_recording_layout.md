...
| `render_mode` | Integer | （选填）画面显示模式：<br><li>0: (Default) Cropped mode.<li>1: Fit mode. |
...
你可以在调用 `start` 或 `updateLayout` 方法时传入 `backgroundColor` 参数设置视频画布颜色。 Supports RGB color codes that start with a hashtag(#) followed by six hexadecimal numbers. 默认值 "#000000" 黑色。
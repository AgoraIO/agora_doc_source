...
- An Agora project with an App ID. The token is not required for this quick start guide. For details, see [Get Started with Agora](https://docs.agora.io/en/Agora%20Platform/get_appid_token?platform=All%20Platforms).
...
1. Download the latest version of the [Agora RDC SDK for Windows](https://docs.agora.io/cn/remote-control/downloads?platform=Windows) and [Agora Video SDK for Windows](https://docs.agora.io/en/Video/downloads?platform=Windows), and unzip the downloaded SDK packages.
...
#define DEFAULT_APP_ID "appId" // Fill the App ID of your project generated on the Agora Console.
#define HOST_USER "rdc_host"  // Fill the userID of the host side.
#define CONTROLLED_USER "rdc_controlled"    // Fill the userID of the controlled side.
    ...
    , public agora::rc::IRemoteControlEventHandler
{
  ...
  private:
    ...
    CString m_szUserId; // The userID of the current user.
};
...
2. Add the following code to the `RDC-TestDlg.cpp` file in your project to implement the logic for both the host side and controlled side in remote controls.
...
void CRDCTestDlg::onUserJoined(agora::rtc::uid_t uid, int elapsed) {
  ...
  // When the host side joins the channel, the RDC starts to render the screen of the controlled side.
  if (m_bMaster) {
...
Run the project on your Windows device. You can see the login screen when you successfully implement a remote control in your app.

## Next steps

To ensure security, you must generate tokens for authentication on your app server in the formal production environment. The Agora RDC SDK uses the message service and authentication logic in the Agora RTM service. For details, see [Authenticate with RTM tokens](./token_server_rtm?platform=All%20Platforms).
# Implement an Agora app token server for Agora Chat
...
You use Agora Chat app tokens to make authenticated REST calls to Agora Chat. To obtain an Agora Chat app token, you retrieve an Agora app token from your app token server and convert it using the Agora app token obtained.
...
> Note: To enable the Agora Chat service, contact support@agora.io.
...
4. In `<Project name>/src/main/resource`, create a `application.properties` file. You need to fill in the App ID and App Certificate your Agora project and set the validity period of the Agora app token you request. 
...
8. Now your token server is successfully deployed. To get an Agora app token from your token server, open http://localhost:8090/chat/app/token in your broswer and you see the token requested.
...
>
> For example, if you set the validity period of the Agora app token in step 4 as 10,000 seconds, and you use this token to convert and obtain an Agora Chat app token after 1000 seconds, the Agora Chat app token is to be expired after 9,900 seconds.

#### Request example

To convert the Agora app token to an Agora Chat app token, you need to send a POST request to the Agora Chat server. 

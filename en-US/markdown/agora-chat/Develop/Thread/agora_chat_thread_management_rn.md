...
- Retrieve the thread list in a chat group by calling [`fetchJoinedChatThreadFromServer`](#retrieve-a-thread-list), and locate the ID of the thread that you want to join.
...
Users can also call `fetchChatThreadWithParentFromServer` to retrieve a paginated list from the server of all the threads in a specified chat group, as shown in the following code sample:
...
To monitor the thread events, users can listen for the callbacks in the `ChatManager` class and add app logics accordingly. If a user wants to stop listening for the callbacks, make sure that the user removes the listener to prevent memory leakage.

...
The offline push function is not available to group with more than 3000 members. To enable the offline push function for such a large group, contact [support@agoro.io](mailto:support@agoro.io) before creating it. Note that it is impossible to enable the offline push function for a large group that is created. The maximum number of members that a group can contain varies with your price plans. For details, see [Pricing Plan Details](./agora_chat_plan#group).

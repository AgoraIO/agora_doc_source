...
Members in the chat room allow list can send chat room messages even when the chat room owner or an admin has muted all the chat room members. However, if a member is already in the chat room mute list, the member still cannot send messages in the chat room even if after being added to the chat room allow list.

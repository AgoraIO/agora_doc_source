...
3. Adds the pagination parameters `pageNum` and `pageSize` in the `fetchConversationsFromServer` method to allow users to get the conversation list from the server with pagination.
4. Adds the `removeMessagesFromServer` method to allow users to delete messages on the server in a unidirectional way.

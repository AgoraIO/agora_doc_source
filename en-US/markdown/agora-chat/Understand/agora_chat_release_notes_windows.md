...
3. Adds the following methods to the `IRoomManagerDelegate` class:
...
4. Adds the following methods to the `IConnectionDelegate` class:
  ...
  - `OnKickedByOtherDevice`：Occurs when the user is forced to log out of the current account from the current device due to login to another device. 
...
5. Adds the following attributes to the `Group` class:           
  ...
  - `IsMemberAllowToInvite`: Whether other group members than the group owner and admins can invite users to join the group.
  - `MaxUserCount`：The maximum number of users that can join the group.
  - `Ext`: Custom group extension information.
  - `IsDisabled`: Whether the group is disabled.     
              
#### Improvements

1. Changed the name space from ChatSDK to AgoraChat.
2. Renamed the `handle` parameter in methods as `callback`.
3. Renamed the `pushmanager` class.
...
7. Adjusted the returned result of the following methods:
  - `importmessage`: The asynchronous callback is triggered to replace the result returned directly when this method is called.
...
8. Renamed the following methods in the `GroupManager` class:
  - `AddGroupWhiteList` is renamed `AddGroupAllowList`.
  - `CheckIfInGroupWhiteList` is renamed `CheckIfInGroupAllowList`.
  - `GetGroupWhiteListFromServer` is renamed `GetGroupAllowListFromServer`.
  - `RemoveGroupWhiteList` is renamed `RemoveGroupAllowList`.            
9. Renamed the following methods in the `RoomManager` class:
  - `AddWhiteListMembers` is renamed `AddAllowListMembers`.
  - `RemoveWhiteListMembers` is renamed `RemoveAllowListMembers`.                
...
12. Made the following adjustments in the `IGroupManagerDelegate` class:
  - `OnAddWhiteListMembersFromGroup` is renamed `OnAddAllowListMembersFromGroup`.
  - `OnRemoveWhiteListMembersFromGroup` is renamed `OnRemoveAllowListMembersFromGroup`.
  - The `reason` parameter is removed from the `OnInvitationAcceptedFromGroup` method.
  - The `groupName` and `decliner` parameters are removed from the `OnRequestToJoinDeclinedFromGroup` method.

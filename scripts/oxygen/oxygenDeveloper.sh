#!/bin/sh
# Oxygen XML Developer Startup script
...
# Classpath
CP=$OXYGEN_HOME/lib/oxygen-basic-utilities.jar:$OXYGEN_HOME/lib/oxygen.jar:$OXYGEN_HOME/lib/oxygenDeveloper.jar:$OXYGEN_HOME/classes:$OXYGEN_HOME
...
#
# Set multiple instances to false if you want to have a single running instance of Oxygen XML Developer when
# repeatedly starting oxygenDeveloper.sh. In this way you can open files by typing:
#    oxygenDeveloper.sh File.xml 
...
set -- \
 ...
 -Dcom.oxygenxml.app.descriptor=ro.sync.exml.DeveloperFrameDescriptor\
...
then
set -- \
 -Xdock:name="Oxygen XML Developer"\
 -Xdock:icon="$OXYGEN_HOME/macOS.resources/Developer.icns"\
 "$@"
fi

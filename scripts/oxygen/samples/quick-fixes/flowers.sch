...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
  ...
  <sch:pattern>
    ...
    <sch:rule context="ul">
      ...
      <sch:report test="count(ancestor::ul) >= 2" sqf:fix="plain" role="error" id="ulNestedLevelID"> Too many levels in
        this list </sch:report>

      <!-- Check that there is more that one lit item in a list -->
      <sch:assert test="count(li) > 1" sqf:fix="addLi plain" role="warn" id="oneLiID"> A list must have more than
        one item </sch:assert>

...
echo "Script file for generating XSL Stylesheet documentation"
echo "using oXygen XML Editor stylesheet documentation generator."
...
"${OXYGEN_JAVA}" -cp "$CP" -Djava.awt.headless=true ro.sync.xsl.documentation.XSLStylesheetDocumentationGenerator "$@"
 
RETCODE=$?
if [ "$RETCODE" -ne "0" ]; then
echo "Documentation generation did not succeed, return code "$RETCODE
exit 1
fi

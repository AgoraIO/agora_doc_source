...
<xsl:stylesheet
  ...
  <xsl:template name="generateCssLinks">
    ...
    <!-- oXygen Patch Start EXM-42549 - adding a / for paths that do not end in a / or \. This is for DITA OT 3.x, it worked without this in DITA OT 2.x -->
    <xsl:variable name="CSSPATH">
      <xsl:choose>
        <xsl:when test="not(ends-with($CSSPATH,'/')) and not(ends-with($CSSPATH,'\'))">
          <xsl:value-of select="concat($CSSPATH,'/')"/>
        </xsl:when>
        <xsl:otherwise>

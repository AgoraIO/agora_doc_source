<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
    ...
    <xsl:template
      match="
      *[contains(@class, ' topic/table ')][not(@id)][*[contains(@class, ' topic/title ')]]">
      <xsl:copy>
	...
	<xsl:template match="*[contains(@class, ' topic/entry ')]">
	    <xsl:copy>
	      ...
	      <xsl:if test="current() = preceding-sibling::processing-instruction('oxy_comment_start')/following-sibling::entry[1]">
	        <xsl:apply-templates select="preceding-sibling::processing-instruction('oxy_comment_start')" mode="processOxygenPIs"/>
	      </xsl:if>
	      <xsl:apply-templates/>
	      <xsl:if test="current() = following-sibling::processing-instruction('oxy_comment_end')/preceding-sibling::entry[1]">
	        <xsl:apply-templates select="following-sibling::processing-instruction('oxy_comment_end')" mode="processOxygenPIs"/>
	      </xsl:if>
	    </xsl:copy>
	</xsl:template>
    ...
    <xsl:template match="*[contains(@class, ' topic/stentry ')]">
      <xsl:copy>
        <xsl:apply-templates select="@*"/>	
        <xsl:if test="current() = preceding-sibling::processing-instruction('oxy_comment_start')/following-sibling::stentry[1]">
          <xsl:apply-templates select="preceding-sibling::processing-instruction('oxy_comment_start')" mode="processOxygenPIs"/>
        </xsl:if>
        <xsl:apply-templates/>
        <xsl:if test="current() = following-sibling::processing-instruction('oxy_comment_end')/preceding-sibling::stentry[1]">
          <xsl:apply-templates select="following-sibling::processing-instruction('oxy_comment_end')" mode="processOxygenPIs"/>
        </xsl:if>
      </xsl:copy>
    </xsl:template>
  ...
  <xsl:template match="
    ...
    processing-instruction('oxy_insert_start')" priority="2">

    <xsl:variable name="isOutsideCell"
      select="
  ...
  <xsl:template match="
    ...
    processing-instruction('oxy_insert_end')" priority="2">

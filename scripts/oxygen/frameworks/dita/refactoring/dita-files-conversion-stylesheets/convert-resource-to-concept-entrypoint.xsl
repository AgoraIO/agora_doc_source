...
<xsl:stylesheet
    ...
    <xsl:import href="convert-resource-to-concept-custom.xsl"/>
    <xsl:import href="handle-convert-to-self.xsl"/>

    <xsl:variable name="root-element" select="'concept'"/>
    <xsl:variable name="public-literal-target" select="'-//OASIS//DTD DITA Concept//EN'"/>
    <xsl:variable name="system-literal-target" select="'concept.dtd'"/>
    ...
    <xsl:variable
        ...
        select="'urn:oasis:names:tc:dita:xsd:concept.xsd'"/>
    <xsl:variable
        name="xml-model-location"
        select="'urn:oasis:names:tc:dita:rng:concept.rng'"/>

...
<xsl:stylesheet
    ...
    <xsl:import href="convert-resource-to-topic-custom.xsl"/>
    <xsl:import href="handle-convert-to-self.xsl"/>

    <xsl:variable name="root-element" select="'topic'"/>
    <xsl:variable name="public-literal-target" select="'-//OASIS//DTD DITA Topic//EN'"/>
    <xsl:variable name="system-literal-target" select="'topic.dtd'"/>
    ...
    <xsl:variable
        ...
        select="'urn:oasis:names:tc:dita:xsd:topic.xsd'"/>
    <xsl:variable
        name="xml-model-location"
        select="'urn:oasis:names:tc:dita:rng:topic.rng'"/>

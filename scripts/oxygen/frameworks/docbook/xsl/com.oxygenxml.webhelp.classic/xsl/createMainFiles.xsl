...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="create-localization-files">
    ...
    <xsl:variable name="jsURL"
      select="concat(oxygen:makeURL(string($OUTPUTDIR)), '/', $jsFileName)"/>
    <xsl:variable name="phpFileName" select="'oxygen-webhelp/resources/localization/strings.php'"/>
    <xsl:variable name="phpURL"
      select="concat(oxygen:makeURL(string($OUTPUTDIR)), '/', $phpFileName)"/>
    <xsl:for-each select="/*">
                ...
                <xsl:text disable-output-escaping="yes">
                  try {
                    ...
                    if ( (parentWindow == "frm" || parentWindow == "contentwin") &amp;&amp;
                      top.window.location.origin == window.location.origin) {
                      
                      var topicLocation = window.location.origin + window.location.pathname.replace(/[^/]*$/, '') +
                                          window.location.hash.substring(1);
                      window.location.href = topicLocation;
                    }
                  } catch (e) {
                    debug("Exception: " + e);
                  }
  ...
  <xsl:template name="create-js-scripts" exclude-result-prefixes="#all">
    <xsl:param name="withFrames"/>
    <script type="text/javascript" src="oxygen-webhelp/resources/js/jquery-3.5.1.min.js"><xsl:comment/></script>

...
<xsl:template match="*[*[position()=1 and contains(local-name(),'info')]]"
  ...
  <!-- START Oxygen Patch (EXM-49696) -->
  <xsl:if test="string-length(normalize-space(string-join(*[position()=1]/*[not(local-name() = 'title')]))) > 0">
    <xsl:apply-templates select="*[position()=1]" mode="topic.prolog.out"/>
  </xsl:if>
  <!-- END Oxygen Patch (EXM-49696) -->
</xsl:template>
...
<xsl:template name="makeTopicId">
  ...
  <xsl:otherwise>
    ...
    <!-- START Oxygen Patch (EXM-49716) -->
    <xsl:if test="string-length(normalize-space($topicName)) = 0">
      <xsl:text>id</xsl:text>
    </xsl:if>
    <!-- END Oxygen Patch (EXM-49716) -->

...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
    ...
    <pattern id="opf.media.overlay.metadata.global.duration">
        <rule context="opf:metadata/opf:meta[@property = 'media:duration']">
            <assert test="matches(normalize-space(.), '^([0-9]+:)?[0-5][0-9]:[0-5][0-9](\.[0-9]+)?$') or matches(normalize-space(.), '^[0-9]+(\.[0-9]+)?(h|min|s|ms)?$')"> The value of the media:duration property must be a valid SMIL clock value. </assert>
        </rule>
    </pattern>
    ...
    <pattern id="opf.itemref.rendition.spread.portrait">
        <rule context="opf:spine/opf:itemref">
            <report test="@properties and (some $token in tokenize(@properties, ' ')                 satisfies (normalize-space($token) eq 'rendition:spread-portrait'))">

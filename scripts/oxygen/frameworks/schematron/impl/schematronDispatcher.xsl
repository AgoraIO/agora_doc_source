            ...
            <xsl:when test="contains(@href, '#')">
                ...
                <xsl:choose>
                    <xsl:when test="$fragment-id!=''">
                        ...
                        <xsl:variable name="doc" select="if (doc-available(resolve-uri($document-uri, base-uri(.)))) then document($document-uri,.) else ()"/>
                        <!-- End patch --> 
                        <xsl:choose>
    ...
    <xsl:template match="iso:extends[@href]" mode="resolveIncludes">
        ...
        <xsl:choose>
            ...
            <xsl:when test="string-length( $fragment-id ) &gt; 0">
                <!-- Oxygen patch - use doc available instead of loading document -->
                <xsl:variable name="theDocument_1" select="if (doc-available(resolve-uri($document-uri, base-uri(/)))) then document($document-uri,/) else ()"/>
                <!-- End patch --> 
            ...
            <xsl:otherwise>
                <!-- Oxygen patch - use doc available instead of loading document -->
                <xsl:variable name="theDocument_2" select="if (doc-available(resolve-uri($document-uri, base-uri(/)))) then document($document-uri,/) else ()"/>
                <!-- End patch --> 

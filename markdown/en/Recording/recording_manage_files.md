...
<td>If you record on the Agora Native SDK, PC, or the Web, each user has an AAC file containing the voice content.</td>
</tr>
<tr><td>UID_HHMMSSMS.mp4</td>
<td>If you record on the Agora Native SDK or PC, each user has an MPEG-4 file. If the user joins and leaves the channel multiple times, this user can have multiple MPEG-4 files containing the video content.</td>
</tr>
<tr><td>UID_HHMMSSMS.webm</td>
<td>If you record on the Web, each user has a WebM file. If the user joins and leaves the channel multiple times, this user can have multiple WebM files containing the video content.</td>
</tr>
...
The recording.log and recording_sys.log files under the ChannelName_HHMMSS directory list any exception or problem that occurred during recording.

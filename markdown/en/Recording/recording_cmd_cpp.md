...
title: Record a Call
platform: CPP
updatedAt: 2020-04-29 16:45:34
---
This page demonstrates how to record a call by using the command line. You can also record calls by calling the API methods, see [Record by API](./recording_api_cpp). The command line and API methods implement the same recording functions. 

Ensure that you integrate the On-premise Recording SDK before proceeding, see [Integrate the SDK](/en/Recording/recording_integrate_cpp).
...
- `appId`: The App ID must be the same as that of the Agora RTC SDK. For details, see [Getting an App ID](/en/Recording/token?platform=All%20Platforms#getting-an-app-id).
...
- `userAccount`: The user account. The maximum length of this parameter is 255 bytes. Supported character scopes are:
...
> From v2.8.0, Agora supports using string user accounts to identify users. We recommend using the same type of user identity in one channel. Set `uid` or `userAccount` according to the type of user identity in the recording channel.
...
- `autoSubscribe`：Sets whether to record the streams of all the users or the specified users.
  - `0`: Record the streams of the specified users.
  - `1`: (Default) Record the streams of all the users.

> If you set this parameter as 0, you should set `subscribeVideoUids` or `subscribeAudioUids` to specify the users whose video or audio you want to record.

- `subscribeVideoUids`: The UIDs or user accounts whose video streams you want to record. You can use this parameter to record the video streams of the specified users. Separate the UIDs or user accounts by a comma with no space. For example, `--subscribeVideoUids 123,456,789`.

  > Takes effect only when `autoSubscribe` is set as 0. 

- `subscribeAudioUids`: The UIDs or user accounts whose audio streams you want to record. You can use this parameter to record the audio streams of the specified users. Separate the UIDs or user accounts by a comma with no space. For example, `--subscribeAudioUids 123,456,789`.
...
- `layoutMode`：Sets the video mixing layout. See [Set Video Layout](/en/Recording/recording_layout_guide) for details.
...
- `maxResolutionUid`: If the recording channel uses the integer UIDs, you can specify the UID of the large video window by this parameter when `layoutMode` is set as `2` (vertical layout), 

- `maxResolutionUserAccount`: If the recording channel uses the string user accounts, you can specify the user account of the large video window by this parameter when `layoutMode` is set as `2` (vertical layout).
...
- `enableCloudProxy`: Enables the cloud proxy. Contact [support@agora.io](mailto:support@agora.io) to use the cloud proxy, see [Use Cloud Proxy](./cloudproxy_recording) for details. 
  - `0`: (Default) Disables the cloud proxy.
  - `1`: Enables the cloud proxy.
...
After the recording is complete, use the transcoding script to merge the recorded files. See [Use the Transcoding Script](./recording_transcoding).

If any error or warning occurs during the recording, see [Error codes](./API%20Reference/recording_cpp/namespaceagora_1_1linuxsdk.html#a11cab69078db26c1f166c68e469dcfcf) and [Warning codes](./API%20Reference/recording_cpp/namespaceagora_1_1linuxsdk.html#a5f37e3fa14fad2982f248d247d76996b).
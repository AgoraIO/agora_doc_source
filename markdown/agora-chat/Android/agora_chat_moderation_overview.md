...
为你的用户提供安全和适当的聊天环境至关重要。Agora Chat 提供了多维度服务可智能审核消息内容并处理不当的用户行为。
...
Agora Chat 提供了多种消息审核和过滤的能力，具体如下：

| 功能           | 描述                                                         |
|--------|-----|
|消息举报|终端用户在客户端对消息进行举报，审核人员可在 Agora Console 查看举报记录，并对消息和消息发送者进行处理。|
|文本审核和图片审核|文本审核和图片审核是基于机器学习模型的消息审核服务。|
|名单过滤|名单过滤服务是根据开发者配置的敏感词词库对消息内容进行过滤的服务。|
|域名过滤|域名过滤是根据开发者配置的域名对消息内容进行过滤的服务。|

### 消息举报

如需使用消息举报功能，开发者需要在客户端集成相应 API，有关详细信息，请参阅以下文档：
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **消息举报**，进入消息举报页面，如下图所示：

   ![报告_zh](https://web-cdn.agora.io/docs-files/1656312948783)

2. 在 **消息报告** 页面，审核人员可以按时间段、会话类型或处理方式过滤消息报告项。他们还可以按用户 ID、组 ID 或聊天室 ID 搜索特定报告项。对于举报，Agora Chat 支持两种处理方式：撤回消息或请求发送者处理消息。

### 消息内容审核 - 文本和图片审核

#### 文本和图片审核介绍

文本和图片审核是 Agora Chat 提供的基于机器学习模型的审核服务，可以扫描用户消息中的违规文本和图片内容，并标记该内容以进行审核。Agora Chat 的文本和图片审核由 [Microsoft Azure Moderator](https://azure.microsoft.com/zh-cn/services/cognitive-services/content-moderator/) 提供支持。该模型可以为任何消息提供置信区间（1-5 分数），分为以下三个类别：

- 成人：在某些情况下，该内容可能被视为色情内容或成人内容。
- 种族：内容可能被视为种族歧视。
- 冒犯的：内容可能被视为攻击或辱骂。

在 Agora 控制台开启文字和图片审核功能后，你可以为每个审核类别设置阈值。当 Microsoft Azure 版主返回的分数达到阈值时，Agora Chat 会阻止该消息。您还可以对在一段时间内达到违规限制的用户进行处罚。审核处罚包括：封禁用户、强制用户下线或删除用户。
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **文本审核规则测试** 和 **图片审核规则测试**，分别可以进入文本和图片审核规则测试页面；
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **名单管理**，进入名单管理页面；
...
1.在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **域名过滤**，进入域名过滤页面：
...
在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **文本审核历史**、**图片审核历史**、**名单过滤历史**、**域名过滤历史**，可分别查看消息审核历史记录。历史记录支持时间段、会话类型、审核结果的筛选；也支持指定发送方 ID、接收方 ID 搜索指定会话的历史记录。
...
<table>
   ...
   <tr>
      <td>强制下线</td>
      <td>强制用户下线，进入离线状态。被强制下线的用户需要重新登录才能正常使用 Chat 服务。</td>
   <tr>
      <td>删除用户</td>
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **用户管理**，进入用户管理页面；
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **群组管理**，进入群组管理页面；
...
1. 在 Console 中依次点击左侧导航栏的 **项目管理** > 对应项目的 **配置** 按钮 > Chat 的 **配置** 按钮 > 左侧二级导航栏的 **聊天室管理**，进入聊天室管理页面；

...
1. 登录声网控制台，在 [项目管理](https://console.agora.io/projects) 页面找到你的项目，然后点击**配置**按钮。
...
在声网控制台开启文字和图片审核功能后，你可以为每个审核类别设置敏感度阈值。若词条的敏感度达到或超过该阈值，即时通讯 IM 会按照配置的规则处理消息，例如，拦截消息。敏感度数值越大意味着审核越严厉，并将拦截更多的不当内容。我们建议你对配置的规则进行测试，以确定设置的敏感度是否符合要求。
...
| 用户处理策略         | 默认不处理。开启后，可以设置时间间隔、触发次数和用户自动处理的策略，触发后该规则后将对用户进行自动处理，详见 [用户管理](#usermanagement)。 |

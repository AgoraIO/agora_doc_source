鉴权指对用户身份进行校验的过程。当你使用 Agora 服务时，例如加入音视频通话、登录即时通讯系统，Agora 会使用 Token 来对用户身份及权限进行核验。
...
下图展示了使用即时通讯 App Token 鉴权的原理：

![](https://web-cdn.agora.io/docs-files/1642583268105)

Agora 即时通讯 App Token 的最长有效期为 **24 小时**。当你调用即时通讯 RESTful 接口时，Agora 即时通讯服务器会对读取包含在 Token 中的信息并进行鉴权。一个即时通讯 App Token 中包含以下信息：

- 你的 Agora 项目的 App ID。
- 你的 Agora 项目的 App 证书。
- 你设置即时通讯 App Token 的有效期。
...
### 使用即时通讯 App Token 调用即时通讯 RESTful API

本节介绍如何获取即时通讯 App Token 并调用即时通讯 RESTful API 在你的 app 中注册一个新用户。

1. 获取 Agora App Token 并将其置换为即时通讯 App Token。打开终端，使用 curl 命令向你的 Agora App Token 服务器发送 GET 请求来获取 Agora App Token：
...
2. 将你获取的 Agora App Token 作为 `Authorization: Bear` 这一请求参数的值来置换出即时通讯 App Token，请求示例如下：
   ...
   响应体参数包含即时通讯 App Token 和该 Token 过期的时间戳（ms），响应示例如下：
...
3. 使用即时通讯 App Token 调用即时通讯 RESTful API 来注册一个新用户。在终端中，使用 curl 命令向即时通讯服务器发送请求注册一个新用户，示例如下：

   ```shell
   # 将 <YourAppToken> 替换为你通过置换获得的即时通讯 IM App Token。
...
本节介绍和即时通讯 App Token 有关的更多信息供你参考。

### 请求路径

当你请求获取即时通讯 App Token 时，需在请求路径中传入以下参数：

| 参数       | 数据类型 | 是否必填 | 描述                                                         |
| ---------- | :------: | :-------- | :----------------------------------------------------------- |
| `OrgName` |  String  | 是  |即时通讯服务分配给每个企业（组织）的唯一标识。         |
| `AppName` |  String  | 是      |即时通讯服务分配给每个 app 的名称。同一企业（组织）下，`AppName` 唯一。 |
| `token`    |  String  | 是      | `Bearer <YourAppToken>`。                                             |

### 请求头部

使用 Agora App Token 置换即时通讯 App Token 的请求头部如下：

...
| `chat`      | Number    | 否       | 单聊消息禁言时长，单位为秒，最大值为 `2147483647`。<ul><li> > `0`：该用户 ID 具体的单聊消息禁言时长。 </li><li> `0`：取消该用户的单聊消息禁言。<br/> - `-1`：该用户被设置永久单聊消息禁言。</li><li> - 其他负值无效。</li></ul> |
| `groupchat` | Number    | 否       | 群组消息禁言时长，单位为秒，规则同上。      |
| `chatroom`  | Number    | 否       | 聊天室消息禁言时长，单位为秒，规则同上。                    |

...
4. 在**功能配置** > **总览** 页面的 **消息功能** 区域开启 **消息回调**。

5. 在**功能配置** > **消息回调** 页面，单击 **添加回调规则**。

![](https://web-cdn.agora.io/docs-files/1681712126525)

![](./images/callback_add_rule.png)

6. 添加发送前回调规则。在 **发送前回调** 页签中设置下表中的字段，然后单击 **保存**。
...
7. 添加发送后回调规则。在**发送后回调**页签中设置下表中的字段，然后单击**保存**。

| 字段           | 是否必填 | 描述                                                         |
| :----------------- | :------- | :----------------------------------------------------------- |
| 规则名称           |  是     | 规则名称。在一个项目下，每条规则都必须有一个唯一的名称。规则名称支持中英文，不能超过 32 个字符。                   |
| 回调服务           |  是     | 该规则适用的会话或事件类型：<ul><li>单聊</li><li>群聊</li><li>聊天室</li><li>在线状态</li><li>消息撤回</li><li>ack事件</li><li>敏感词监测</li><li>好友关系操作</li><li>内容审核</li><li>群组和聊天室操作</li></ul>     |

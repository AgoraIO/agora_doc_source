...
| `strategy`    | Number | 推送策略：<ul><li>`0`：厂商通道优先，失败时走声网通道。</li><li>`1`：只走声网通道。该情况下，若用户在线，则直接推送；若用户离线，消息保留 7 天，超过该期限则丢弃消息。</li><li>（默认）`2`：只走厂商通道。若推送失败，直接丢弃推送消息。</li><li>`3`：声网通道优先，失败时走厂商通道。</li></ul> | 否   |  

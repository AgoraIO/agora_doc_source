本文展示如何调用 Agora 即时通讯 RESTful API 实现用户体系建立和管理。包括对用户注册、获取、修改、删除、封禁、解禁、强制下线等。
...
| `Authorization` | String | App 管理员的鉴权 token，格式为 Bearer ${token}，其中 Bearer 是固定字符，后面加英文空格，再加获取到的 token 值。 | 是       |
...
| 参数     | 说明                                                         |
| :------- | :----------------------------------------------------------- |
| `username` | 数据格式为 "用户 ID": "当前在线状态"。例如用户 user1 的在线状态：如果该用户在线，则返回 "user1": "online"；如果不在线，则返回 "user1": "offline"。 |
...
|   参数    | 类型   | 是否必需 | 描述         |
| :-------: | :----- | :------- | ------------ |
| `owner_username` | String |   必需| 要获取离线消息数的用户 ID。|

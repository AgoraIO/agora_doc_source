...
即时通讯提供消息云存储服务，包括历史消息、漫游消息和离线消息。
...
- 除获取历史消息记录文件 RESTful API，所有即时通讯 RESTful API 的调用频率都针对单个 IP 地址。
- 获取历史消息记录文件 API 的调用频率上限为针对单个 app 1 次/分钟。
- 除部分 API 接口有特殊说明外，API 调用频率限制默认为 100 次/秒，具体以各模块的下表为准。
...
| :------------------- | :----- | :------------------- | :------------------------- |
| 查询单个用户的详情         | GET    | /{org_name}/{app_name}/users/{username}                      | 100 次/秒/App Key  |
| 批量查询用户详情         | GET    | /{org_name}/{app_name}/users                                 | 100 次/秒/App Key          |
...
| 获取单个用户在线状态     | GET    | /{org_name}/{app_name}/users/{username}/status               | 100 次/秒/App Key          |
| 批量获取用户在线状态 | POST   | /{org_name}/{app_name}/users/batch/status                    | 50 次/秒/App Key           |
| 获取用户的离线消息数量       | GET    | /{org_name}/{app_name}/users/{owner_username}/offline_msg_count | 100 次/秒/App Key          |
| 查询离线消息的投递状态   | GET    | /{org_name}/{app_name}/users/{username}/offline_msg_status/{msg_id} | 100 次/秒/App Key          |
| 封禁用户             | POST   | /{org_name}/{app_name}/users/{username}/deactivate           | 100 次/秒/App Key          |
| 解禁用户             | POST   | /{org_name}/{app_name}/users/{username}/activate             | 100 次/秒/App Key          |
...
| 获取聊天历史消息记录 | GET    | /{org_name}/{app_name}/chatmessages/${time}          | 10 次/分钟/App Key                                           |
...
| RESTful API 接口          | 方法   | 接口 URL                    | 接口最高调用频率（默认值） |
| :------------------------------ | :----- | :--------------------- | :------------------------- |
| 获取 app 中所有的群组 | GET    | /{org_name}/{app_name}/chatgroups?limit={N}&cursor={cursor}         | 100 次/秒/App Key    |
| 获取单个用户加入的所有群组      | GET    | /{org_name}/{app_name}/users/{username}/joined_chatgroups?pagesize={}&pagenum={}              | 50 次/秒/App Key           |
...
| 解除成员禁言             | DELETE | /{org_name}/{app_name}/chatgroups/{group_id}/mute/{member_id} | 100 次/秒/App Key          |
...
| RESTful API 接口           | 方法   | 接口 URL                                                    | 接口最高调用频率（默认值） |
| :---------------------- | :----- | :---------------------------------------------------------- | :------------------------- |
| 获取 app 中所有的聊天室 | GET    | /{org_name}/{app_name}/chatrooms?limit={N}&cursor={cursor}                            | 50 次/秒/App Key           |
| 获取用户加入的聊天室    | GET    | /{org_name}/{app_name}/users/{username}/joined_chatrooms    | 50 次/秒/App Key           |
| 查询指定聊天室详情         | GET    | /{org_name}/{app_name}/chatrooms/{chatroom_id}              | 100 次/秒/App Key          |
| 创建聊天室          | POST   | /{org_name}/{app_name}/chatrooms                            | 50 次/秒/App Key           |
...
| RESTful API 接口              | 方法   | 接口 URL                                                     | 接口最高调用频率（默认值） |
| :------------------------- | :----- | :----------------------------------------------------------- | :------------------------- |
| 分页获取聊天室成员         | GET    | /{org_name}/{app_name}/chatrooms/{chatroom_id}/users?pagenum={N}&pagesize={N}      | 100 次/秒/App Key          |
...
| 禁言聊天室单个成员             | POST   | /{org_name}/{app_name}/chatrooms/{chatroom_id}/mute          | 100 次/秒/App Key          |
| 禁言聊天室全体成员         | POST   | /{org_name}/{app_name}/chatrooms/{chatroom_id}/ban           | 100 次/秒/App Key          |
| 解除聊天室禁言成员         | DELETE | /{org_name}/{app_name}/chatrooms/{chatroom_id}/mute/{member} | 100 次/秒/App Key          |
| 解除聊天室全员禁言         | DELETE | /{org_name}/{app_name}/chatrooms/{chatroom_id}/ban           | 100 次/秒/App Key          |
| 获取超级管理员列表         | GET    | /{org_name}/{app_name}/chatrooms/super_admin?pagenum={N}&pagesize={N}                | 100 次/秒/App Key          |
...
| RESTful API 接口      | 方法   | 接口 URL        | 接口最高调用频率（默认值） |
| :------------------- | :----- | :------------------------ | :----------- |
| 创建/添加 Reaction         | POST   | /{org_name}/{app_name}/reaction/user/{username}   | 100 次/秒  |
| 根据消息 ID 获取 Reaction     | GET    | /{org_name}/{app_name}/reaction/user/{username}?msgIdList={N,M}&msgType={msgType}&groupId={groupId}  | 100 次/秒  |
| 删除 Reaction     | DELETE | /{org_name}/{app_name}/reaction/user/{username}?msgId={msgId}&message={message}| 100 次/秒  |
| 根据消息 ID 和表情 ID 获取 Reaction 信息 | GET    | /{org_name}/{app_name}/reaction/user/{username}/detail?msgId={msgId}&message={message}&limit={limit}&cursor={cursor} | 100 次/秒        |

### 子区管理

| RESTful API 接口      | 方法   | 接口 URL        | 接口最高调用频率（默认值） |
| :------------------- | :----- | :------------------------ | :----------- |
| 创建子区  | POST     | /{org_name}/{app_name}/thread    | 100 次/秒    |
| 修改子区  | PUT     | /{org_name}/{app_name}/thread/{thread_id}    | 100 次/秒    |
| 删除子区  | DELETE     | /{org_name}/{app_name}/thread/{thread_id}    | 100 次/秒    |
| 分页获取 app 中的子区  | GET  | /{org_name}/{app_name}/thread?limit={limit}&cursor={cursor}&sort={sort} | 100 次/秒  |
| 分页获取单个用户加入的所有子区  | GET     | /{org_name}/{app_name}/threads/user/{username}?limit={limit}&cursor={cursor}&sort={sort}    | 100 次/秒    |
| 分页获取单个用户在指定群组中加入的所有子区  | GET  | /{org_name}/{app_name}/threads/chatgroups/{group_id}/user/{username}?limit={limit}&cursor={cursor}&sort={sort} | 100 次/秒    |
| 分页获取子区成员列表  | GET     | /{org_name}/{app_name}/thread/{thread_id}/users?limit={N}&cursor={cursor}   | 100 次/秒    |
| 用户批量加入子区  | POST     | /{org_name}/{app_name}/thread/{thread_id}/users   | 100 次/秒    |
| 批量删除子区成员  | DELETE     | /{org_name}/{app_name}/threads/{thread_id}/users   | 100 次/秒    |
...
4. **发布多源采集的视频流**
   ...
   - `publishFourthScreenTrack`：发布第四个屏幕采集的视频。
     ...
     <div class="alert info">目前 SDK 支持在同一时间、同一 `RtcConnection` 中发布多路音频流、一路视频流。</div>
...
2. 为帮助用户了解更多类型的远端视频状态改变的原因，`onRemoteVideoStateChanged` 回调中新增了 `RemoteVideoStateReasonCodecNotSupport` 枚举，表示本地的视频解码器不支持对收到的远端视频流进行解码。

...
其中主要节点如下：
    ...
    | 7    | **Check Permission**   | 自定义   | (可选) 检查是否已获取极速直播所需的系统权限，如访问摄像头和麦克风等。<div class="alert note">该节点仅适用于 Android 和 iOS。</div> |
    | 8    | **Init Rtc Engine**     | 自定义   | 创建并初始化 RTC 引擎。  |
    | 9    | **Un Init Rtc Engine**  | 自定义   | 离开频道并释放资源。     |

[1]：原生节点为蓝图自带的节点，可以直接添加调用。
[2]：自定义节点非蓝图自带，需要用户在创建自定义函数后才能添加对应节点。
...
3. 远端主播或观众加入频道时，触发 `onUserJoined` 回调，创建远端视图，uid 为远端发送来的 uid，视频源类型为 **VIDEO_SOURCE_REMOTE** (网络获取的远端视频)。
![](https://web-cdn.agora.io/docs-files/1689839388183)

4. 远端主播或观众离开频道时，触发 `onUserOffline` 回调，释放远端视图。

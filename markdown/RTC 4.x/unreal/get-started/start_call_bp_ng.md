...
本节介绍如何使用声网视频 SDK 在你的项目中实现视频通话功能。

### 创建 UMG

为直观地体验视频通话，需根据应用场景创建以下控件：
   - 本地和远端的视频窗口
   - 加入和离开频道的按钮
...
在 **My Blueprint** 面板中，双击 **Graphs** > **EventGraph** 打开事件图表，其中已有 **Event BeginPlay** (游戏开始) 和 **Event End Play** (游戏结束) 两个事件节点。接下来，创建视频通话相关的事件节点以及对应的函数和变量，并将它们连接起来以实现视频通话的逻辑和交互，如下图所示：
...
1. 创建并实现 **MakeVideoView** 函数，在本地或远端主播加入频道时加载视图。
...
按照以下步骤来测试你的视频通话项目：
   ...
   2. 在 **Unreal Editor** 中，点击播放按钮来运行你的项目，然后点击 **JoinChannel** 加入视频通话。
   3. 邀请一位朋友通过另一台设备来使用相同的 App ID、频道名、Token 加入频道。你们可以听见、看见对方。
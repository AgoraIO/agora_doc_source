...
在开始录制时，你可以通过 [`start`](./cloud_recording_api_start?platform=RESTful) 方法中的相关参数设置订阅的黑白名单。在录制过程中，你可以通过 [`update`](./cloud_recording_api_update) 方法中的 `streamSubscribe` 参数更新订阅名单。

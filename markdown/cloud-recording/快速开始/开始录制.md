...
在开始录制前，必须先调用 [`acquire`](https://docs.agora.io/cn/cloud-recording/cloud_recording_api_acquire) 方法获取一个云端录制 resource ID。
...
<a name="start"></a>
### start：开始录制
获得 resource ID 后，在五分钟内调用 [`start`](https://docs.agora.io/cn/cloud-recording/cloud_recording_api_start) 方法加入频道开始录制。你可以选择录制模式为单流录制或合流录制。
...
调用 [`stop`](https://docs.agora.io/cn/cloud-recording/cloud_recording_api_stop) 方法停止录制。

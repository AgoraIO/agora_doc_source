...
本次发布在全球范围内支持延时转码功能。你需要在调用 `acquire` 接口时将 `scene` 字段设置为 `2`（延时转码），设置为该场景后，录制服务会在录制后 24 小时内对录制文件进行转码生成 MP4 文件，并将 MP4 文件上传至你指定的第三方云存储（不支持七牛云）。请注意第三方云存储的 `region` 必须与调用 `acquire` 时设置的 `region`一致。详见[实现单流录制](https://docs.agora.io/cn/cloud-recording/cloud_recording_individual_mode?platform=All%20Platforms)。

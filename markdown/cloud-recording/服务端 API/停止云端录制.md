...
| `clientRequest` | JSON Object | 特定的客户请求参数，对于该请求包含以下参数：<li>`async_stop`：（选填）Boolean 类型，设置 `stop` 方法的响应机制：<ul><li>`true`：同步。调用 `stop` 后立即收到响应。</li><li>（默认）`false`：异步。调用 `stop` 后，需等待所有录制文件上传至第三方云存储方可收到响应。</li></ul></li> |
...
- `serverResponse`: JSON Object，服务器返回的具体信息。该字段中的子元素与你在 [`start`](./cloud_recording_api_start?platform=RESTful) 请求中的设置有关。单流视频截图场景只会返回 `uploadingStatus` 字段，页面录制场景只会返回 `extensionServiceState` 字段，其他场景返回 `fileListMode`、`fileList` 和 `uploadingStatus` 字段。
    ...
    - `uploadingStatus`：String 类型，当前录制上传的状态。
        ...
        - `"unknow"`：未知状态。
    - `extensionServiceState`: JSON Array，由[上传服务](#上传服务)和[页面录制服务](#页面录制服务)的状态组成的数组。

...
- `subscribeAudioUids`：（选填）JSON Array 类型，由 UID 组成的数组，指定订阅哪几个 UID 的音频流。如需订阅全部 UID 的音频流，则无需设置该参数。数组长度不得超过 32，不推荐使用空数组。详见[设置订阅名单](./cloud-recording/cloud_recording_individual_mode?platform=All%20Platforms#设置订阅名单)。
- `unSubscribeAudioUids`: （选填）JSON Array 类型，由 UID 组成的数组，指定不订阅哪几个 UID 的音频流。云端录制会订阅频道内除指定 UID 外所有 UID 的音频流。数组长度不得超过 32，不推荐使用空数组。[设置订阅名单](./cloud-recording/cloud_recording_individual_mode?platform=All%20Platforms#设置订阅名单)。
- `subscribeVideoUids`:（选填）JSON Array 类型，由 UID 组成的数组，指定订阅哪几个 UID 的视频流。如需订阅全部 UID 的视频流，则无需设置该参数。数组长度不得超过 32，不推荐使用空数组。[设置订阅名单](./cloud-recording/cloud_recording_individual_mode?platform=All%20Platforms#设置订阅名单)。
- `unSubscribeVideoUids`: （选填）JSON Array 类型，由 UID 组成的数组，指定不订阅哪几个 UID 的视频流。云端录制会订阅频道内除指定 UID 外所有 UID 的视频流。数组长度不得超过 32，不推荐使用空数组。[设置订阅名单](./cloud-recording/cloud_recording_individual_mode?platform=All%20Platforms#设置订阅名单)。

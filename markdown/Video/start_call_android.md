...
2. 将视频 SDK 集成到你的项目中。 针对 Agora SDK v3.5.0 或更高版本，可按照以下步骤将 SDK 集成到 mavenCentral。 针对 v3.5.0 之前的各 SDK 版本，请参考[集成 SDK 的其他方法](https://docs.agora.io/en/Video/start_call_android?platform=Android#othermethods)。
...
1. 检查 app 是否有正确的权限。 如果已授予权限，调用 `initializeAndJoinChannel` 加入视频通话频道。
...
Agora 不推荐在生产环境中手动生成 Token。 请参考[使用 Token 鉴权](https://docs.agora.io/en/Video/token_server?platform=Android)，了解如何从你的服务器中获取 Token 并开始视频通话。
...
1. 在 [SDK 下载](https://docs.agora.io/en/Video/downloads?platform=Android)页面下载最新版本的 Agora 视频 SDK ，并解压。

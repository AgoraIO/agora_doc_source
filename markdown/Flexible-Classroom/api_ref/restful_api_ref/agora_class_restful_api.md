...
| 参数    | 类型   | 是否必填  | 描述                                                                                                                                                                                                             |
| :------ | :----- | :----- |:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `dirty` | Object | 非必填   | 用户踢人标记，包含以下字段：<ul><li>`state`: Integer 型，踢人状态：<ul><li>`1`: 被踢，无法进入课堂。</li><li> `0`: 未被踢，可以进入课堂。</li></ul><li>`duration`: Number 型，临时踢人持续时间，单位为秒，从被踢出时开始计时。当被踢持续时长结束后，用户自动恢复为未被踢状态。若不传duration，则永久踢出。</li></ul> |

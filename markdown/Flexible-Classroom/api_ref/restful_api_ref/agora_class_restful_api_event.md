## 概述
本页列出通过灵动课堂云服务[查询所有课堂事件接口](./agora_class_restful_api#查询所有课堂事件)获取到的所有事件类型。

...
2. 在 Android Studio 中编译并运行 `fullcode/android` 项目。运行成功后，即可看到你的 Scene 的登录界面。输入房间名、用户名，选择一种班型，然后点击加入。
...
3. 在 Android Studio 中编译并运行 `CloudClass-Android` 项目。运行成功后，即可看到你的 Scene 的登录界面。输入房间名、用户名，选择一种班型，然后点击加入。

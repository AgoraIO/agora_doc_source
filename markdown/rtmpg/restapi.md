你可以通过 RESTful API 创建推流密钥，使用该密钥将在线媒体流作为直播视频源输入到声网频道内。同时，也可以通过 RESTful API 开启并配置在线流的转码能力。
...
以下内容以使用 OBS 推流软件输入 RTMP 协议的媒体流为例：

![](https://web-cdn.agora.io/docs-files/1687156698789)

- 服务器：可以使用声网统一域名，也可以绑定用户自己的域名。
  <a id="region"></a>
  - 声网统一域名分区域使用 `rtls-ingress-prod-{region}.goramdn.com`。其中支持的区域（即 `{region}` 字段中填的值）如下：
    - `cn`：中国大陆
    - `ap`：除中国大陆以外的亚洲区域
    - `na`：北美
    - `eu`：欧洲

  - 使用用户自己的域名，请联系[声网技术支持](/cn/Agora%20Platform/ticket)进行配置，生效后方可使用。
...
默认情况下，推流到 RTMP 网关服务之后，会将视频不经转码推送到声网频道。如果需要设置转码参数，需要使用 RESTful API 设置相关参数，详见 [Update：设置服务配置信息](#update-设置服务配置信息)。
...
以上示例代码依赖 `msgpack-lite` 模块。如果你还没有安装，执行 `npm install msgpack-lite` 后运行 `node app.js` 即可。
...
<div class="alert note">推流码被删除之后，就不能再使用该推流码进行推流。但如果推流码正在使用中，删除推流码并不会影响当前正在推送的流。如果要立即禁用该条流，请联系声网技术支持。</div>
...
- `transcoding`（选填）转码设置，JSON Object 型。包括以下字段：
  - `video`（选填）视频转码设置，JSON Object 型。包括以下字段：
    ...
    - `bitrate`（选填）编码码率，Number 型。取值范围取决于设置的 `width`，`height`，`fps` 值，若超出范围会重置到有效范围（具体设置请联系声网技术支持确认）。
  - `audio`（选填）JSON Object 型。包括以下字段：
    - `enabled`（必填）是否开启音频转码，Boolean 型。默认值为 `true`（开启）。
    - `profile`（选填）编码音频场景，Number 型。默认为 `0`，表示：48 KHz 采样率，音乐编码，单声道，编码码率最大值为 64 Kbps。如要设置其它场景，请联系声网技术支持确认。
- `simulcastStream`（选填）视频大小流功能，用于提高弱网环境下观众端的自适应切换能力，JSON Object 型。包括以下字段：
  ...
  - `bitrate`（选填）小流码率，Number 型。取值范围取决于设置的 `width`，`height`，`fps` 值，若超出范围会重置到有效范围（具体设置请联系声网技术支持确认）。
...
| 409 Conflict            | Resource with the same name already exists.                  | 并发请求过多。 | 采取退避策略重试。 |
...
- 请确保声网频道场景为直播。
- 如果你需要同一个频道内推送多条不同的流，请确保每个推流码的 `uid` 都是唯一的，详见 [`uid` 参数说明](#uid)。

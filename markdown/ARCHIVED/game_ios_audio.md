...
<tr><td>appId</td>
<td>Agora 为应用程序开发者签发的厂商秘钥。详见 <a href="../Agora Platform/token"><span>获取 App ID</span></a></td>
</tr>
...
<img alt="../_images/windows_audio_api_live.png" src="https://web-cdn.agora.io/docs-files/cn/windows_audio_api_live.png" style="width: 230.4px; height: 336.8px;"/>
...
<div><ul>
<li>AUDIO_PROFILE_DEFAULT = 0: 默认设置。通信场景下为 1，直播场景下为 2</li>
...
<li>安全要求高: 将值设置为 Token 值。 如果你已经启用了 App Certificate, 请务必使用 Token。 关于如何获取 Token，详见 <a href="../Agora Platform/token"><span>密钥说明</span></a> 。</li>
</ul>
</td>
</tr>
<tr><td>channelId</td>
...
默认值为 100.0。取值越小，音效的音量越低。onon</p>
</td></tr>
<tr><td>返回值</td>
<td><ul>
<li>0: 方法调用成功</li>
...
<tr><td>返回值</td>
<td><ul>
<li>&lt; 0: 创建数据流失败的错误码 <a href="#f2" id="id35">[3]</a></li>
<li>&gt; 0: 数据流 ID</li>
</ul>
</td>
</tr>
...
> 返回的错误码是负数，对应错误代码和警告代码里的正整数。例如返回的错误码为 -2，则对应错误代码和警告代码里的 2: ERR_INVALID_ARGUMENT 。
...
> 如果之前有调用过 `muteAllRemoteAudioStreams` (true) 对所有远端音频进行静音，在调用本 API 之前请确保你已调用 m`uteAllRemoteAudioStreams` (false) 。 `muteAllRemoteAudioStreams` 是全局控制，`muteRemoteAudioStream` 是精细控制。
...
<tr><td>volume</td>
<td><p>设置耳返音量，取值范围在 [0.100]，默认值为 100</p>
</td>
</tr>
</tr>
</tbody>
...
agora::IAudioDeviceManager 接口类提供用于测试音频设备的相关接口。可通过实例化 IAudioDeviceManager 类来获取 IAudioDeviceManager 接口。
...
<tr><td>返回值</td>
<td><ul>
<li>调用成功时，IAudioDeviceCollection 对象，包含所有的播放设备;</li>
<li>调用失败时，返回 NULL</li>
</ul>
</td>
</tr>
...
该方法返回一个 IAudioDeviceCollection 对象，包含系统中所有的录音设备。通过 IAudioDeviceCollection 对象，应用程序可以枚举录音设备。 在使用结束后，应用程序需调用IAudioDeviceCollection::release()方法销毁返回的对象。
...
<tr><td>返回值</td>
<td><ul>
<li>调用成功时，IAudioDeviceCollection 对象，包含所有的录音设备;</li>
<li>调用失败时，返回 NULL</li>
</ul>
</td>
</tr>
...
<tr><td>deviceId</td>
<td>播放设备的 Device ID。可通过 enumeratePlaybackDevices() 获取。插拔设备不会影响 deviceId</td>
</tr>
<tr><td>返回值</td>
<td><ul>
<li>0: 方法调用成功</li>
...
<tr><td>deviceId</td>
<td>录音设备的 Device ID。可通过 enumerateRecordingDevices() 获取。插拔设备不会影响 deviceId</td>
</tr>
<tr><td>返回值</td>
<td><ul>
<li>0: 方法调用成功</li>
...
### 设置播放设备音量 (setPlaybackDeviceVolume)
...
### 获取播放设备音量 (getPlaybackDeviceVolume)
...
### 设置麦克风音量 (setRecordingDeviceVolume)
...
### 获取麦克风音量 (getRecordingDeviceVolume)
...
### 获取播放设备静音状态 (getPlaybackDeviceMute)
...
### 获取麦克静音状态 (getRecordingDeviceMute)
...
### 开启声卡采集 (enableLoopbackRecording)
...
该方法测试播放设备是否能正常工作。SDK播放指定的音频文件，测试者如果能听到声音，说明播放设备能正常工作。
...
该方法测试麦克风是否能正常工作。启动测试后，SDK 通过 onAudioVolumeIndication 回调方法向应用程序上报音量信息。
...
该方法停止麦克风测试。调用startRecordingDeviceTest()后，必须调用该方法停止测试。
...
agora::IAudioDeviceCollection 接口类获取音频设备相关的数据。
...
### 设置 App 音量 (setApplicationVolume)
...
### 获取 App 的当前音量 (getApplicationVolume)
...
<tr><td>mute</td>
<td>
<div>是否设置 App 为静音</div>
<ul>
<li>True：App 设为静音</li>
<li>False：App 设为非静音</li>
</ul>
</td>
</tr>
<tr><td>返回值</td>
...
### 获取 App 当前的静音状态 (isApplicationMute)
...
<tr><td>mute</td>
<td>
<div>获取 App 当前的静音状态</div>
<ul>
<li>True：App 为静音状态</li>
<li>False：App 为非静音状态</li>
</ul>
</td>
</tr>
<tr><td>返回值</td>
...
该回调提示频道内谁在说话以及说话者的音量。默认禁用。可以通过启用说话者音量提示 `enableAudioVolumeIndication` 方法开启；开启后，无论频道内是否有人说话，都会按方法中设置的时间间隔返回提示音量。

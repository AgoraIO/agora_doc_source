...
AccessToken2 allows users to specify the validity period of an RTM token. The validity period can be a maximum of 24 hours. Besides, this release adds the `rtmKitTokenPrivilegeWillExpire` callback triggered when a token is about to expire in 30 seconds. Upon receiving this callback, you can generate a new RTM token on your app server, and pass the new RTM token to the SDK; otherwise, the connection state of the SDK switches from connected to aborted due to `AgoraRtmConnectionChangeReasonTokenExpired (9)`. For details, see [RTM Token expiration](./token_server_rtm#rtm-token-expiration).

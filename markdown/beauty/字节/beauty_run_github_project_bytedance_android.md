声网在 GitHub 上提供开源 [BeautyAPI](https://github.com/AgoraIO-Community/BeautyAPI/tree/main) 示例项目供你参考。本文介绍如何快速跑通该示例项目，体验美颜效果。
...
1. 进入声网控制台的[项目管理](https://console.shengwang.cn/project)页面。
2. 在项目管理页面，点击**创建**按钮。
3. 在弹出的对话框内输入**项目名称**、**使用场景**，然后在**鉴权机制**中选择**调试模式：APP ID**。
...
1. 联系{#TODOcompany}技术支持获取最新的美颜 SDK、美颜资源、美颜证书。

2. 下载并解压美颜 SDK，然后将 SDK 中如下文件添加到秀场直播示例项目对应的文件路径下：{#TODOcompany}
...
运行美颜项目前，你需要在 `gradle.properties` 文件中添加如下参数：

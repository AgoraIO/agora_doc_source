...
| `conversion`   | 文件转换配置对象。灵动课堂会对后缀名 `"ppt"`、`"pptx"`、`"doc"`、`"docx"`、`"pdf"` 的文件默认开启文件转换，以用于课堂内白板展示。包含以下字段：<ul><li>`type`: 转换类型：</li><ul><li>`"static"`: 转换为静态图片。灵动课堂对`"ppt"`、`"doc"`、`"docx"`、`"pdf"` 文件开启静态转换。</li><li>`"dynamic"`: 转换为动态 HTML。灵动课堂对 `"pptx"` 文件开启动态转换。</li></ul><ul> |
| `url`          | 文件下载地址。当文件后缀名非 `"ppt"`、`"pptx"`、`"doc"`、`"docx"` 或 `"pdf"` 时，必须设置 `url`，`scenes` 可为空。 |
| `scenes`       | 转换后的文件下载配置。当文件后缀名为 `"ppt"`、`"pptx"`、`"doc"`、`"docx"` 或 `"pdf"` 时，必须设置 `scenes`。 |

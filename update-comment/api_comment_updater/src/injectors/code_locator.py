...
from ..utils.file_utils import find_files_by_patterns, read_file_content


class CodeLocator:
    """代码定位器，负责在代码仓库中定位API位置"""
    ...
    def __init__(self, repo_config: Dict[str, Any], platform_config: Dict[str, Any]):
        ...
        logger.debug("初始化代码定位器: 平台={}, 仓库路径={}", self.platform, self.repo_path)
    ...
    def _search_by_enum_name(self, enum_name: str, search_files: List[str]) -> Optional[Tuple[str, int]]:
        ...
        enum_patterns = [
            ...
            # Java枚举类模式：public static class EnumName
            rf"\b(?:public\s+|private\s+|protected\s+)?(?:static\s+)?class\s+{re.escape(enum_name)}\s*{{",
            rf"^\s*(?:public\s+|private\s+|protected\s+)?(?:static\s+)?class\s+{re.escape(enum_name)}\b"
        ]
    ...
    def _clean_signature(self, signature: str) -> str:
        ...
        # 平台隔离：根据平台使用不同的清理策略
        if self.platform in ['android-ng', 'ios-ng']:
            return self._clean_java_signature(signature)
        else:
            return self._clean_cpp_signature(signature)
    
    def _clean_java_signature(self, signature: str) -> str:
        """
    ...
    def _clean_cpp_signature(self, signature: str) -> str:
        """
    ...
    def _clean_code_line_for_matching(self, line: str) -> str:
        ...
        # 平台隔离：根据平台使用不同的清理策略
        if self.platform in ['android-ng', 'ios-ng']:
            return self._clean_java_code_line(line)
        else:
            return self._clean_cpp_code_line(line)
    
    def _clean_java_code_line(self, line: str) -> str:
        """
        清理Java代码行，移除注解等
    ...
    def _clean_cpp_code_line(self, line: str) -> str:
        """
        清理C++代码行
    ...
    def _is_attribute_definition(self, line: str, attribute_name: str) -> bool:
        """
        判断是否为属性定义行
        ...
        # 属性定义通常包含类型和属性名
        # 支持各种格式：Optional<int> name; int name; AUDIO_PROCESSING_CHANNELS name;
        patterns = [
            rf"\b{re.escape(attribute_name)}\s*[;=]",  # name; 或 name =
            rf"\b{re.escape(attribute_name)}\s*$",     # 行末的 name
        ]
    ...
    def _is_enum_value_definition(self, line: str, value_name: str) -> bool:
        ...
        Returns:
            bool: 是否为枚举值定义
        """
        # 平台隔离：根据平台使用不同的枚举值识别策略
        if self.platform in ['android-ng', 'ios-ng']:
            return self._is_java_enum_value_definition(line, value_name)
        else:
            return self._is_cpp_enum_value_definition(line, value_name)
    
    def _is_java_enum_value_definition(self, line: str, value_name: str) -> bool:
        """
        Java枚举值定义检测
    ...
    def _is_cpp_enum_value_definition(self, line: str, value_name: str) -> bool:
        """
        C++枚举值定义检测
    ...
    def _find_nearest_parent_class(self, lines: List[str], api_line_index: int) -> Optional[str]:
        ...
        # 平台隔离：根据平台使用不同的类检测策略
        if self.platform in ['android-ng', 'ios-ng']:
            return self._find_nearest_java_parent_class(lines, api_line_index)
        else:
            return self._find_nearest_cpp_parent_class(lines, api_line_index)
    
    def _find_nearest_java_parent_class(self, lines: List[str], api_line_index: int) -> Optional[str]:
        """
    ...
    def _find_nearest_cpp_parent_class(self, lines: List[str], api_line_index: int) -> Optional[str]:
        """
    ...
    def _extract_clean_api_name(self, api_name: str) -> str:
        ...
        支持的清理规则：
        - C++/Java重载标识: "getDevice [1/2]" -> "getDevice"
        - OC选择器参数: "setClientRole:options:" -> "setClientRole"
    ...
    def _normalize_signature_enhanced(self, signature: str) -> str:
        ...
        # logger.debug("签名标准化: {} -> {}", signature.replace('\n', '\\n'), normalized)
        return normalized
    ...
    def _extract_candidate_signature(self, lines: List[str], start_index: int) -> Optional[str]:
        ...
        Returns:
            Optional[str]: 提取的签名，如果不是函数定义返回None
        """
    ...
    def _search_attribute_enhanced(self, lines: List[str], class_start: int, class_end: int, attribute_name: str) -> Optional[int]:
        ...
        Returns:
            Optional[int]: 属性行索引，如果未找到返回None
        """
    ...
    def _is_attribute_definition_enhanced(self, line: str, attribute_name: str) -> bool:
        ...
        # 平台隔离：根据平台使用不同的识别策略
        if self.platform in ['android-ng', 'ios-ng']:
            return self._is_java_attribute_definition(line, attribute_name)
        else:
            return self._is_cpp_attribute_definition(line, attribute_name)
    
    def _is_java_attribute_definition(self, line: str, attribute_name: str) -> bool:
        """
        Java属性定义检测
    ...
    def _is_cpp_attribute_definition(self, line: str, attribute_name: str) -> bool:
        """
        C++属性定义检测

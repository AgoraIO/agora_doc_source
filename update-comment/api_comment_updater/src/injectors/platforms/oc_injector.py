...
class OcInjector(BaseInjector):
    ...
    def inject_api_comment(self, api_data: Dict[str, Any]) -> bool:
        ...
        logger.debug("开始注入Objective-C API注释: {}", api_name)
        ...
        if success:
            logger.info("成功注入Objective-C API注释: {} -> {}:{}", api_name, file_path, line_number)
        else:
            logger.error("注入Objective-C API注释失败: {}", api_name)
    ...
    def inject_class_comment(self, class_data: Dict[str, Any]) -> bool:
        ...
        logger.debug("开始批量注入Objective-C类注释: {}", class_name)
        
        # 定位类在代码中的位置
        location = self.code_locator.locate_class(class_data)
        if not location:
            logger.warning("未能定位Objective-C类 {}, 跳过注入", class_name)
            return False
        ...
        for comment_item in normalized_data.get("comments", []):
            ...
            if comment_type == "class_description" and comment_lines:
                ...
                if success:
                    ...
                    logger.debug("成功注入Objective-C类描述注释: {}", class_name)
                else:
                    logger.error("注入Objective-C类描述注释失败: {}", class_name)
                    return False
                break
    ...
    def inject_enum_comment(self, enum_data: Dict[str, Any]) -> bool:
        ...
        logger.debug("开始批量注入Objective-C枚举注释: {}", enum_name)
        ...
        for comment_item in normalized_data.get("comments", []):
            ...
            if comment_type == "enum_description" and comment_lines:
                ...
                if success:
                    ...
                    logger.debug("成功注入Objective-C枚举描述注释: {}", enum_name)
                else:
                    logger.error("注入Objective-C枚举描述注释失败: {}", enum_name)
                    return False
                break
    ...
    def _find_existing_comment_start(self, lines: List[str], target_line: int) -> Optional[int]:
        """
        查找目标行前的现有注释块的开始位置（Objective-C优化版）
        ...
        """
        # 如果目标行是API定义的续行，需要找到API定义的第一行
        actual_target_line = self._find_api_definition_start(lines, target_line)
        
        # 从目标行的上一行开始向上搜索
        current_line = actual_target_line - 1
        ...
        while current_line >= 0:
            ...
            # 检查是否为注释块
            comment_start = self._find_single_comment_start(lines, current_line)
            if comment_start is not None:
                comment_blocks.append((comment_start, current_line))
        ...
        if comment_blocks:
            # 按开始行号排序，返回最早的
            comment_blocks.sort(key=lambda x: x[0])
            return comment_blocks[0][0]
        
        return None
    
    def _find_api_definition_start(self, lines: List[str], target_line: int) -> int:
        """
        查找API定义的第一行
        ...
        while current_line >= 0:
            ...
            # 检查是否为API定义行
            if self._is_api_definition_line(line_content):
                return current_line
            
            # 检查是否为API定义的续行
            if self._is_api_continuation_line(line_content):
                current_line -= 1
                continue
    ...
    def _is_api_definition_line(self, line: str) -> bool:
        """
        检查是否为API定义行
        ...
        Returns:
            bool: 是否为API定义行
        """
        # 检查Objective-C方法定义（不需要以分号结尾，因为可能是多行定义）
    ...
    def _is_api_continuation_line(self, line: str) -> bool:
        """
        检查是否为API定义的续行
        ...
        # 检查是否包含Objective-C方法参数
        if ':' in line and ('(' in line or ')' in line):
            return True
        
        return False
    
    def _find_single_comment_start(self, lines: List[str], end_line: int) -> Optional[int]:
        """
        查找单个注释块的开始位置
    ...
    def _inject_comment_at_location(self, file_path: str, line_number: int, 
        ...
        try:
            ...
            while True:
                ...
                if start_line is not None:
                    ...
                    if end_line is not None:
                        ...
                        deleted_blocks.append((start_line, end_line))
                        logger.debug("删除了注释块: {} - {}", start_line, end_line)
                    else:
            ...
            if deleted_blocks:
                logger.debug("总共删除了 {} 个注释块", len(deleted_blocks))
            ...
            logger.debug("成功注入Objective-C注释到 {}:{}", file_path, line_number)
            return True
            
        except Exception as e:
            logger.error("注入Objective-C注释失败 {} -> {}:{}: {}", target_name, file_path, line_number, str(e))
            return False
    
    def _find_existing_comment_start(self, lines: List[str], target_line: int) -> Optional[int]:
        """
        查找目标行前的现有注释块的开始位置（参考C++逻辑）

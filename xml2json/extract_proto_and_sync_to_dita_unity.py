...
def extract_rn_proto(cpp_code, content):
    ...
    if cpp_core is not None:
        ...
        for code in result:
            print(code)
            unity_code.append(code)

        # print(unity_code)

    else:
        unity_code = ["There are no corresponding names available"]
...
def extract_cpp_struct_unity_class(cpp_code, content):
    ...
    if cpp_core is not None:
        ...
        # Avoid Catastrophic Backtracking: https://www.regular-expressions.info/catastrophic.html
        # unity_proto_re = r'(class|mixin|abstract class)\s{0,10}' + re.escape(text) + r'\s{0,10}\{[A-Za-z_\s\n\?\[\]\.,;\{\}\(\)\<\>\=\$@]{0,500}\}'
        # unity_proto_re = r'class\s{0,10}' + re.escape(text) + r'\s{0,10}\{[A-Za-z_;\s\n\?\[\]\{\}\(\)<>=\$\n,@:\.]{0,5000}\}'
        ...
        # unity_proto_re = text + r"\([A-Za-z_\s\n\?\n,\.,]{0,1000}\);"
...
def extract_cpp_enum_unity_class(cpp_code, content):
        ...
        for file in files:
            if file.endswith(".cs"):
                with open(os.path.join(root, file), encoding='utf8', mode='r') as f:
                    ...
                    with open(decomment_code_location + "//" + "concatenated.cs", encoding='utf8', mode='a') as f1:
                        print("Writing to concatenated file...")
                        f1.write(text)

    with open(decomment_code_location + "//" + "concatenated.cs", encoding='utf8', mode='r') as f:
        ...
        for file, code in dictionary.items():
            ...
            if name.startswith("api_"):
                ...
                dart_struct = extract_cpp_struct_unity_class(code, content)
                print(dart_struct)
                dart_file_list.append(file)
        ...
        for file, proto in dict111.items():
            ...
            for child in root.iter('*'):
                if child.get("props") == "unity" and child.tag == "codeblock":
                    if proto != "There are no corresponding names available" and child.text == "":
                        child.text = proto
